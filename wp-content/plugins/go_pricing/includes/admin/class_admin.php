...
class GW_GoPricing_Admin {	
	...
	public function load_includes() {
		...
		include_once ( $this->plugin_path . 'includes/admin/class_admin_page_addons.php');	
	...
	public function enqueue_admin_scripts() {
		...
		if ( array_key_exists( $screen->id, self::$screen_hooks ) ) {
			...
			wp_enqueue_script( self::$plugin_slug . '-admin-scripts', $this->plugin_url . 'assets/admin/js/go_pricing_admin_scripts.js', array( 'jquery' ), self::$plugin_version );						
			wp_enqueue_script( self::$plugin_slug . '-spectrum', $this->plugin_url . 'assets/lib/spectrum/spectrum.js', array( 'jquery' ), self::$plugin_version, true );
			wp_enqueue_script( self::$plugin_slug . '-codemirror', $this->plugin_url . 'assets/lib/codemirror/codemirror_custom.js', array( 'jquery' ), self::$plugin_version, true );			
	...
	public function register_menu_pages() {
		...
		// Add-ons page
		self::add_submenu_page(
			__( 'Add-ons', 'go_pricing_textdomain' ),
			__( 'Add-ons', 'go_pricing_textdomain' ),
			isset( $general_settings['capability'] ) ? $general_settings['capability'] : 'manage_options', 
			self::$plugin_slug . '-addons',
	...
	public static function addons_page() {
		...
		$page = new GW_GoPricing_AdminPage_Addons( __METHOD__ );
		$page->title( sprintf( 'Go Pricing - %s',  __( 'Add-ons', 'go_pricing_textdomain' ) ) );
		echo $page->render();
