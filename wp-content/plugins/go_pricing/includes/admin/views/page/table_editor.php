...
if ( $this->is_ajax === false ) {
	if ( !empty( $_GET['id'] ) && !empty( $table_data ) ) {		
		$postid = (int)$_GET['id'];	
	}
} else {
...
if ( !empty( $postid ) && empty( $table_data ) ) $table_data = GW_GoPricing_Data::get_table( $postid );
				...
				<div class="gwa-abox-content-wrap gwa-abox-tab-contents">					
					...
					<div class="gwa-abox-content gwa-abox-tab-content">
						...
						<table class="gwa-table">                           
							...
							<tr>
								<th><label><?php _e( 'Full-Height Background?', 'go_pricing_textdomain' ); ?></label></th>
								<td><p><label><span class="gwa-checkbox<?php echo !empty( $table_data['equalize']['fullh-bg'] ) || empty( $table_data ) ? ' gwa-checked' : ''; ?>" tabindex="0"><span></span><input type="checkbox" name="equalize[fullh-bg]" tabindex="-1" value="1" <?php echo !empty( $table_data['equalize']['fullh-bg'] ) || empty( $table_data ) ? ' checked="checked"' : ''; ?>></span></label></p></td>
								<td class="gwa-abox-info"><p class="gwa-info"><i class="fa fa-info-circle"></i><?php _e( 'Whether set full height for column background layer.', 'go_pricing_textdomain' ); ?></p></td>
							</tr>                             																																		
						</table>																																																														
					</div>
