...
if ( !defined( 'WP_USE_THEMES' ) ) {
	...
	$go_pricing_id = !empty( $_GET['id'] ) ? (int)$_GET['id'] : NULL;
} else {
...
<body>
<?php
if ( !is_user_logged_in() || is_null( $go_pricing_id ) || empty( $_GET['nonce'] ) || ( !empty( $_GET['nonce'] ) &&  wp_verify_nonce( $_GET['nonce'], $instance['plugin_base'] . '-preview' ) === false ) ) :
	?>
	<div id="go-pricing-forbidden"><span class="icon-exclamation-triangle"></span><?php _e( 'Oops, Forbidden!', 'go_pricing_textdomain' ); ?></div>
	<?php 
else :
?>
<div id="go-pricing-preview"><?php echo do_shortcode( '[go_pricing postid="' . $go_pricing_id . '" margin_bottom="0" preview="true"]' ); ?></div>
<?php 
endif;
wp_footer(); 
?>
