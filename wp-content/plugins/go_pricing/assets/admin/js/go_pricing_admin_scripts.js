...
jQuery(document).ready(function($, undefined) {	
	...
	/ ---------------------------------------------------------------------- */	
		...
		var setHelpCookie = function() {
			if (document.cookie.indexOf('go_pricing[settings][help]['+userSettings.uid+']') == -1) {
				document.cookie='go_pricing[settings][help]['+userSettings.uid+']=1';
				$goPricingAdmin.find('.gwa-pcontent').attr('data-help', 1);
			}
		}
		setHelpCookie();
		...
		$goPricingAdmin.on('click', '[data-action="delete-col"], [data-action="clone-col"], [data-action="expand-col"]', function(e) {
			...
			if ($this.data('action')=="delete-col") {
				...
				if (confirm($this.data('confirm'))){
					...
					$newCol.find('.gwa-col-box').removeClass('gwa-current');
					setColIndex();
					updateColCnt();
				};					
	...
	/ ---------------------------------------------------------------------- */
		...
		function addFavColor(color, $elem) {
			...
			if ($.inArray(color, colors) == -1 && color != '' ) colors.splice(0,0, color);
	...
	$goPricingAdmin.on('click', '.gwa-message-close', function(e) {

		var $this = $(this),
			$message = $this.closest('.gwa-message, .gwa-banner'),
			id = $message.data('id') || 0;

		document.cookie='go_pricing_msg_'+id+' = true; expires=Fri, 31 Dec 9999 23:59:59 GMT';
  		$message.slideUp();
